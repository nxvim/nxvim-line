...
mode         The mode label (NORMAL / INSERT / VISUAL / …). The data source
             for the mode colour (see Themes). Also labels the selection-first
             Helix modes (HELIX / HELIX-SEL, opt-in via `:helix`) and vim's
             Select mode (SELECT / S-LINE); Helix-normal takes the normal
             palette, the extend/select modes the visual palette.
